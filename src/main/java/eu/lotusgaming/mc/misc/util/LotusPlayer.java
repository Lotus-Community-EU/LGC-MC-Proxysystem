...
import eu.lotusgaming.mc.misc.ScoreboardState;
import net.md_5.bungee.api.connection.ProxiedPlayer;

public class LotusPlayer {
	
	private String uuid, clan, name, nick, currentLastServer, language, playerGroup, customDateFormat, customTimeFormat, timeZone, countryCode, colorCode;
	...
	/**
	 * Constructor class
	 * @param playerName - needed to get the correct data for according player.
	 */
	public LotusPlayer(String playerName) {
		try(PreparedStatement ps = MySQL.getConnection().prepareStatement("SELECT * FROM mc_users WHERE name = ?")){
			...
			if(rs.next()) {
				...
				this.colorCode = getColorCodeFromDB(rs.getString("playerGroup"));
				this.proxiedPlayer = Main.main.getProxy().getPlayer(rs.getString("name"));
	 ...
	 */
	public LotusPlayer(ProxiedPlayer player) {
	...
	private String getColorCodeFromDB(String roleName) {
		try (PreparedStatement ps = MySQL.getConnection().prepareStatement("SELECT colour FROM core_ranks WHERE name = ?")) {
	...
	/**
	 ...
	 /**
	  * Gets the Role Color from this player.
	  * @return the role color in HEX
	  */
	 ...
	 /**
	  * Returns the ProxiedPlayer Instance - might be null!
	  * @return the ProxiedPlayer Instance
	  */
	 public ProxiedPlayer getPlayerInstance() {
